...
def declare_logic():
    ...
    # Automatically count the number of accesses linked to each user.
    Rule.count(derive=User.access_count, as_count_of=Access)

    # Automatically count the number of invitations linked to each user.
    Rule.count(derive=User.invitation_count, as_count_of=Invitation)

    # Count the number of users associated with each group.
    Rule.count(derive=Group.user_count, as_count_of=User, where=lambda row: row.group_id == Group.id)

    # Count the number of users associated with each profile.
    Rule.count(derive=Profile.user_count, as_count_of=User, where=lambda row: row.profile_id == Profile.id)

    # Ensure that the invitation start and end dates do not exceed access dates.
    Rule.constraint(validate=Invitation, as_condition=lambda row: some_logic_to_verify_date_within_access(row), error_msg='Invitation dates exceed access period.')
